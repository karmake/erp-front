...
const handleToggleDrawer = (id) => {
  setDrawerPedido((prevDrawerPedido) => (prevDrawerPedido === id ? null : id));
};

const renderAdditionalInfo = (pedido) => {
    return (
        <div style={{ padding: "10px", background: "#f1f1f1", borderRadius: "5px" }}>
...
const renderDrawer = (pedido) => {
  const totalFaturado = pedido.items.reduce((acc, item) => acc + parseFloat(item.precoTotal || 0), 0).toFixed(2);
  const quantidadeItens = pedido.items.length;

  return (
    <>
      <Overlay show={drawerPedido === pedido.id} onClick={() => setDrawerPedido(null)} />
      <Drawer show={drawerPedido === pedido.id}>
        <h4>Informações adicionais</h4>
        <p><strong>Prazo:</strong> {pedido.prazo ? format(new Date(pedido.prazo), "dd 'de' MMMM 'de' yyyy", { locale: ptBR }) : "Não informado"}</p>
        <p><strong>Transportadora:</strong> {pedido.transportadora || 'Não especificada'}</p>
        <p><strong>Código do pedido:</strong> {pedido.codigoMarketplace}</p>
        ...
        <div style={{ marginTop: "20px", borderTop: "1px solid #e0e0e0", paddingTop: "10px" }}>
          ...
          <p><strong>Total faturado:</strong> R$ {totalFaturado}</p>
        </div>

        {/* Botão para imprimir etiqueta se disponível */}
        {pedido.etiqueta && (
          <Button variant="primary" onClick={() => imprimirEtiquetaEnvio(pedido.etiqueta)}>
            Imprimir etiqueta
          </Button>
        )}
        <Button variant="secondary" onClick={() => setDrawerPedido(null)}>Fechar</Button>
      </Drawer>
    </>
  );
};
  ...
  return (
    ...
    <Container>
      ...
      >
        ...
        />
        <Tab eventKey="novo" title={`Novo (${countPedidosByStatus("novo")})`} />
        <Tab
      ...
      <Alert style={{ marginBottom: 10, paddingBottom: 10 }} variant="warning">
...
>
  ...
  {pedido.items.length > 1 && !expandedOrders.includes(pedido.id) && (
    <div
      style={{
        display: "flex",
    ...
    >
      ...
      <div>
        <p>{pedido.items.length} produto(s)</p> {/* Mostrar o número de produtos */}
      </div>
  ...
  {expandedOrders.includes(pedido.id) && (
    <PedidoBody>
      ...
      <PedidoAcoes>
        ...
        {pedido.numeroNfe && (
          <PedidoLink onClick={() => handleShowNfeModal(pedido)}>
            Ver Dados da NF-e
          </PedidoLink>
        )}
        {pedido.etiqueta && (
          <PedidoLink onClick={() => imprimirEtiquetaEnvio(pedido.etiqueta)}>
  ...
  {pedido.items.length === 1 && (
    <PedidoBody>
      <div style={{ display: "flex", alignItems: "center" }}>
        <ProdutoImage
          src={imagens[pedido.items[0].id] || "https://via.placeholder.com/150"}
          alt={pedido.items[0].produto}
        />
        <ProdutoInfo>
          <p>
            {pedido.items[0].produto} x <strong>{pedido.items[0].quantidade}</strong>
          </p>
      ...
      <Modal show={nfeModalShow} onHide={handleCloseNfeModal}>
        ...
        <Modal.Body>
          {selectedNfe ? (
            <>
              <p>
                <strong>ID do Pedido:</strong> {selectedNfe.codigoMarketplace}
              </p>
              <p>
